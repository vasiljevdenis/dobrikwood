jQuery(function (g) {
  ...
  var v = {
          ...
          (v.xhr = g.ajax({
            ...
            success: function (e) {
              if (e && !0 === e.reload) window.location.reload();
                ...
                let shippingMethods = document.querySelectorAll('ul#shipping_method li');

                shippingMethods.forEach((el, i) => {
                  if (first) {
                    if (i != shippingMethods.length - 1) {
                      el.style.display = 'none';
                    }
                    shippingMethods[shippingMethods.length - 1].querySelector('input').click();
                  } else {
                    if (i != shippingMethods.length - 1) {
                      el.style.display = 'none';
                    } else {
                      if (typeof lastMethod === 'number') {
                        shippingMethods[lastMethod].style.display = 'list-item';
                      } else {
                        shippingMethods[0].style.display = 'list-item';
                      }
                    }
    ...
    submit: function () {
      ...
      return (
        ...
        (!1 !== e.triggerHandler("checkout_place_order") &&
          !1 !== e.triggerHandler("checkout_place_order_" + v.get_payment_method()) &&
          (e.addClass("processing"),
            ...
            g.ajax({
              ...
              success: function (e) {
                ...
                try {
                  if ("success" !== e.result) throw "failure" === e.result ? "Result failure" : "Invalid response";
    ...
    m = {
      init: function () {
        ...
        let deliveryPoint;
        let searchControl;
        const el = document.querySelector('p#billing_email_field');
        el.insertAdjacentHTML('afterend', '<p style="font-weight: 700;" id="addressName"></p>');
        el.insertAdjacentHTML('afterend', '<div id="dp_map" style="height: 400px; width: 100%;"></div>');
        el.insertAdjacentHTML('afterend', '<p id="addressLabel" style="font-weight: 700;">Адрес <abbr class="required" title="обязательно" style="color: red; text-decoration: none;">*</abbr></p>');
        document.querySelector('p#billing_adr_meg_field').style.display = 'none';
        const addressInp = document.querySelector('p#billing_adr_meg_field input#billing_adr_meg');
        const addressName = document.querySelector('p#addressName');
        ymaps.ready(initMap);
        function initMap() {
          map = new ymaps.Map('dp_map', {
            center: [39.201312, 51.663005],
            zoom: 11,
          ...
          let myPlacemark = new ymaps.GeoObject({
            properties: {
              balloonContent: "<div style=\"margin-bottom: 10px;\"><strong>YOKO SUSHI</strong></div>\n<p>Пушкинская улица, 8, Воронеж</p>"
            },
            geometry: {
              type: "Point",
              coordinates: [39.196065, 51.662551]
            }
          });
        ...
        function onZonesLoad(json) {
          ...
          let deliveryZones = ymaps.geoQuery(json).addToMap(map);
          ...
          deliveryPoint.events.add('dragend', function () {
            ...
            map.setZoom(12, { duration: 500 });
          });

          function highlightResult(obj) {
            let subtotal = +document.querySelector('.cart-subtotal span.woocommerce-Price-amount.amount').innerHTML.replace(/[^0-9]/g, '');
            ...
            if (polygon) {
              ...
              deliveryPoint.options.set('iconColor', 'black');
              addressInp.value = 'none';
              addressName.innerHTML = 'Самовывоз';
              let shippingMethods = document.querySelectorAll('ul#shipping_method li');
              shippingMethods[shippingMethods.length - 1].querySelector('input').click();
            }
            ...
            function setData(obj) {
              ...
              let free = polygon.properties.get('free');
              let delivery = polygon.properties.get('delivery');
              let id = polygon.properties.get('id');
              let shippingMethods = document.querySelectorAll('ul#shipping_method li');
              if (subtotal >= free) {
                shippingMethods.forEach((el, i) => {
                  if (i < shippingMethods.length - 2) {
                    el.style.display = 'none';
                  } else if (i === shippingMethods.length - 2) {
                    lastMethod = i;
                ...
                shippingMethods.forEach((el, i) => {
                  if (i === shippingMethods.length - 3 - id) {
                    lastMethod = i;
                    el.querySelector('input').click();
                  } else if (i != shippingMethods.length - 1) {
                    el.style.display = 'none';
                  }
                });
              }
  ...
  v.init(), e.init(), t.init(), o.init(), m.init();
});
